# Clean up restart files - ESM1.5 version
...
parser.add_argument('--archivedir', dest='archivedir',
                    default=None,
                    help='archive directory (default is args.disk/$PROJECT/$USER/archive')
...
if not args.noatm:    
    # Atmospheric files
    files = sorted(restartdir.joinpath('atm').glob("%s.astart-[0-9]*" % args.runid))
    flist = list(files)[1:-keeplast]
    # Remove anything that's not 0101_00
    rlist = []
    for f in flist:
        if f.name.endswith('0101'):
...
if not args.noocn:
    ...
    if len(files) == 0:
        ...
        ocntar=True
    #files = sorted(restartdir.joinpath('ocn').glob("csiro_bgc.res.nc-[0-9]*"))
    flist = list(files)[1:-keeplast]
    ...
    for f in rlist:
        if ocntar: 
            ...
            rlistall += sorted(restartdir.joinpath('ocn').glob("csiro*" + suffix))
    if args.verbose > 1:
...
if not args.noice:
    ...
    # end of run date and iced.YYYYMMDD using start of
    # following run date
    ...
    files = sorted(restartdir.joinpath('ice').glob("iced.[0-9-]*"))
    flist = list(files)[1:-keeplast]
    # Remove anything that's not 0101
    rlist = []
    for f in flist:
        if f.name.endswith('0101'):
            if args.decadal_start >= 0:
